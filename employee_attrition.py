...
def normalize_features(data, features):
    ...
    normalized_features = (features_df - features_df.min()) / (features_df.max() - features_df.min())

    return normalized_features

def define_fuzzy_variables():
    """Define fuzzy variables and membership functions."""
    over_time = ctrl.Antecedent(np.arange(0, 2, 1), 'OverTime_Yes')  # Binary: Yes=1, No=0
    years_at_company = ctrl.Antecedent(np.arange(0, 1.1, 0.1), 'YearsAtCompany')
    age = ctrl.Antecedent(np.arange(0, 1.1, 0.1), 'Age')
    work_life_balance = ctrl.Antecedent(np.arange(0, 1.1, 0.1), 'WorkLifeBalance')
    # gender = ctrl.Antecedent(np.arange(0, 2, 1), 'Gender_Male')
    environment_satisfaction = ctrl.Antecedent(np.arange(0, 1.1, 0.1), 'EnvironmentSatisfaction')
    ...
    years_at_company['short'] = fuzz.trimf(years_at_company.universe, [0, 0, 0.5])
    years_at_company['medium'] = fuzz.trimf(years_at_company.universe, [0, 0.5, 1])
    years_at_company['long'] = fuzz.trimf(years_at_company.universe, [0.5, 1, 1])
    ...
    work_life_balance['poor'] = fuzz.trimf(work_life_balance.universe, [0, 0, 0.5])
    work_life_balance['average'] = fuzz.trimf(work_life_balance.universe, [0, 0.5, 1])
    work_life_balance['good'] = fuzz.trimf(work_life_balance.universe, [0.5, 1, 1])

    # gender['female'] = fuzz.trimf(gender.universe, [0, 0, 0.5])
    # gender['male'] = fuzz.trimf(gender.universe, [0.5, 1, 1])

    environment_satisfaction['low'] = fuzz.trimf(environment_satisfaction.universe, [0, 0, 0.5])
    environment_satisfaction['medium'] = fuzz.trimf(environment_satisfaction.universe, [0, 0.5, 1])
    environment_satisfaction['high'] = fuzz.trimf(environment_satisfaction.universe, [0.5, 1, 1])

    job_involvement['low'] = fuzz.trimf(job_involvement.universe, [0, 0, 0.5])
    job_involvement['medium'] = fuzz.trimf(job_involvement.universe, [0, 0.5, 1])
    job_involvement['high'] = fuzz.trimf(job_involvement.universe, [0.5, 1, 1])

    return over_time, years_at_company, age, work_life_balance, environment_satisfaction, job_involvement

def define_fuzzy_rules(over_time_var, years_var, age_var,
                       work_var, env_var, job_var):
...
def predict_attrition(simulation, input_data):
    ...
    simulation.input['OverTime_Yes'] = input_data.get('OverTime_Yes', 0)  # Default to no overtime
    ...
    simulation.input['EnvironmentSatisfaction'] = input_data.get('EnvironmentSatisfaction', 0)
    simulation.input['JobInvolvement'] = input_data.get('JobInvolvement', 0)
    # simulation.input['Gender_Male'] = input_data.get('Gender_Male', 1)  # Default to male
    ...
    for feature in features:
        if feature in input_data:
            ...
            if max_val - min_val != 0:
                normalized_data[feature] = (input_data[feature] - min_val) / (max_val - min_val)
            else:
                normalized_data[feature] = 0  # If max == min, normalization is 0
        else:
            normalized_data[feature] = 0  # Default value if feature not provided
...
def main():
   ...
   data_file_path = 'WA_Fn-UseC_-HR-Employee-Attrition.csv'
   ...
   try:
        ...
        over_time_var, years_var, age_var, work_var, env_var, job_var = define_fuzzy_variables()
        ...
        input_data_example={
        'OverTime_Yes': 1, # Convert to binary for over time
        'YearsAtCompany': 0,  # min: 0; max: 40
        'Age': 18, # min: 18; max: 60
        'WorkLifeBalance': 1, # min: 1; max: 4
        'EnvironmentSatisfaction': 1,  # min: 1; max: 4
        'JobInvolvement': 1, # min: 1; max: 4
        }
        normalized_input=normalize_input(input_data_example,processed_data,features_to_use)
        likelihood=predict_attrition(simulation_instance, normalized_input)
        print(f'Predicted Attrition Likelihood: {likelihood:.2f}')


        input_data_example={
        'OverTime_Yes': 0, # Convert to binary for over time
        'YearsAtCompany': 40,  # min: 0; max: 40
        'Age': 60, # min: 18; max: 60
        'WorkLifeBalance': 4, # min: 1; max: 4
        'EnvironmentSatisfaction': 4,  # min: 1; max: 4
        'JobInvolvement': 4, # min: 1; max: 4
        }
        normalized_input=normalize_input(input_data_example,processed_data,features_to_use)
        likelihood=predict_attrition(simulation_instance, normalized_input)
        print(f'Predicted Attrition Likelihood: {likelihood:.2f}')
